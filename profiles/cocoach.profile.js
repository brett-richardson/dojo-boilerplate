...
var profile = {
	...
	selectorEngine: 'lite',  // 'acme', // FIXME: May need to add this back in for IE
	...
	staticHasFeatures: {
		'dojo-trace-api'        : 1, // 0 // The trace & log APIs are used for debugging the loader, so we don’t need them in the build
		'dojo-log-api'          : 1, // 0,
		'dojo-publish-privates' : 1, // 0, // This causes normally private loader data to be exposed for debugging, so we don’t need that either
	...
	resourceTags: {
		test: function( filename, mid ){ // Files that contain test code.
			return false;
		},

		copyOnly: function( filename, mid ){ // Files that should be copied as-is without being modified by the build system.
			return copyOnly( mid );
		},

		amd: function( filename, mid ){ // Files that are AMD modules.
			return !copyOnly( mid ) && /\.js$/.test( filename );
		},

		miniExclude: function( filename, mid ){ // Files that should not be copied when the “mini” compiler flag is set to true.
			return mid in {
				'app/profile': 1
			};
		}
